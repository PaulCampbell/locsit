...
TweetSchema.methods.setRatingForMaps = function(callback) {
   var r = 0;
...
TweetSchema.methods.setLocationForMaps = function( callback){
    var tweet = this;
    this.setRatingForMaps(function(rating){
        if(tweet.embedded_postcode)
          {
              // it's a uk postcode
              geocoder.geocode(tweet.embedded_postcode, function ( err, location ) {
                if(err)
                {
                    callback(err)
                }
          ...
          else
          {
              // geocoded tweet
              if(tweet.latitude)
                  tweet.latitude_for_map = tweet.latitude;
              if(tweet.longitude)
                  tweet.longitude_for_map = tweet.longitude;
              callback(null, tweet);
          }
    });
